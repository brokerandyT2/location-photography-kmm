// core/src/commonMain/kotlin/com/x3squaredcircles/core/infrastructure/repositories/WeatherRepository.kt
...
import com.x3squaredcircles.photographyshared.db.Database
...
class WeatherRepository(
    ...
    override suspend fun getRecentAsync(count: Int): Result<List<Weather>> {
        return try {
            val validCount = maxOf(1, minOf(count, 100)).toLong()
            
            withContext(Dispatchers.IO) {
                val weatherList = database.weatherQueries.selectRecent(validCount).executeAsList()
                val result = weatherList.map { it.toWeatherEntity() }
                Result.success(result)
            }
        } catch (ex: Exception) {
        ...
        return try {
            withContext(Dispatchers.IO) {
                database.weatherQueries.insert(
                    ...
                    isDeleted = if (weather.isDeleted) 1L else 0L
                )
                
                // Get the inserted weather with its new ID
                val insertedId = database.weatherQueries.lastInsertRowId().executeAsOne()
                val created = weather.copy(id = insertedId.toInt())
    ...
    override suspend fun upsertAsync(weather: Weather): Result<Weather> {
        return try {
            withContext(Dispatchers.IO) {
                val existing = getByLocationIdAsync(weather.locationId)
                
                when (existing) {
                    is Result.Success -> {
                        val updated = existing.data.copy(
                            latitude = weather.latitude,
                            longitude = weather.longitude,
                            timezone = weather.timezone,
    ...
    override suspend fun cleanupExpiredDataAsync(maxAgeMillis: Long): Result<Int> {
        return try {
            withContext(Dispatchers.IO) {
                val cutoffTime = System.currentTimeMillis() - maxAgeMillis
                val deletedCount = database.weatherQueries.cleanupExpired(cutoffTime).executeAsOne()
                Result.success(deletedCount.toInt())
            }
        } catch (ex: Exception) {
            Result.failure("$WEATHER_ERROR_DATABASE_ERROR: ${ex.message}")
        }
